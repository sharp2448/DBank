...
contract('dBank', ([deployer, user]) => {
  ...
  describe('testing token contract...', () => {
    describe('success', () => {
      it('checking token name', async () => {
        expect(await token.name()).to.be.eq('Lugashs DBank Currency');
      });

      it('checking token symbol', async () => {
        expect(await token.symbol()).to.be.eq('LDBC');
      });
  ...
  describe('testing deposit...', () => {
    ...
    describe('failure', () => {
      ...
      it('depositing should be rejected, deposit should be already active', async () => {
        await dbank.deposit({ value: 10 ** 16, from: user }).should.be.rejectedWith(EVM_REVERT); //0.01 ETH
      });
    });
  });
